...
//Metodo para llamar a la clase Accion y agregar personajes en el juego
	public void addAccion(Accion a){
		actors.add(a);
	}
//Metodo para llamar a los metodos de la  clase Memoria 
...
//Metodo para inicializar enemgios y al personaje en el juego.
	public void initWorld(){
		...
		for(int i=0;i<4;i++){
			...
			m.setX((int)(Math.random()+1*(Inicio.WIDTH-35)));
			m.setY(i*40);
			m.setVx((int)(Math.random()*10-5));
			actors.add(m);
		}
		jugador=new Jugador(this); 		
		jugador.setX(Inicio.WIDTH/2);
		jugador.setY(Inicio.PLAY_HEIGHT-1*jugador.getHeight());
		sonido.loopSound("audio.wav");
	}
